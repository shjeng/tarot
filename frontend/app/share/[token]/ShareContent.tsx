...
export function ShareContent({ data }: { data: SharedReading }) {
  ...
  const cards = isSpread
    ? (data.request_data.cards ?? [])
    : data.request_data.card ? [data.request_data.card] : [];
          ...
          <div className="flex justify-center gap-6 flex-wrap py-2">
            {cards.map((c, idx) => (
              <div key={idx} className="flex flex-col items-center gap-2">
                ...
                />
                {labels[idx] ? (
                  <span className="text-xs text-muted-foreground tracking-wide">{labels[idx]}</span>
                ) : null}
                <span className="text-sm text-foreground/70 font-medium">{c.nameKo}</span>
              </div>
            ))}
          </div>
        )}
